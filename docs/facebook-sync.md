...
Pulls the latest posts from the official LGU Albay Facebook Page, categorizes
...
1. **Get an Editor role on the page.** Ask the LGU Albay page admin (via Meta
   Business Suite → Settings → People) to add the BetterAlbay account/app as an
...
| Name              | Type         | Value                                               |
| ----------------- | ------------ | --------------------------------------------------- |
| `FTP_SERVER`      | Secret       | cPanel FTP host (e.g. `ftp.betteralbay.org`)       |

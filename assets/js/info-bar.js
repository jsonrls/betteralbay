...
const WeatherService = {
  ...
  async fetchWeather() {
    try {
      const response = await fetch(
        `https://api.open-meteo.com/v1/forecast?latitude=${CONFIG.ALBAY_LAT}&longitude=${CONFIG.ALBAY_LON}&current_weather=true`
      );
      ...
      if (data.current_weather && typeof data.current_weather.temperature === 'number') {
        return {
          ...
          timestamp: Date.now(),
          location: {
            lat: CONFIG.ALBAY_LAT,
            lon: CONFIG.ALBAY_LON,
